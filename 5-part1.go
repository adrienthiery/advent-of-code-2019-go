...
func executeStep(program *[]int, baseIndex int, input int, output *int) int {
	...
	if opcode == 1 {
		...
		// write output
		targetIndex := (*program)[baseIndex + 1]
		*output = (*program)[targetIndex]
		fmt.Println("Writing to output value of index: ", targetIndex, "value is:", (*program)[targetIndex], "(output is: ", *output, ")")
		returnIndex += 2
	} else {
		// error
		fmt.Println("opcode")
...
func main() {
	...
	executeProgram(program, testInput2, &testOutput)
